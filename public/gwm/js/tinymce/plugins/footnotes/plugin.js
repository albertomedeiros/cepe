tinymce.PluginManager.add('footnotes', function(editor) {
    ...
    function showDialog() {
        var selectedNode = editor.selection.getNode(), name = '',
            isFootNotes = selectedNode.tagName == 'SPAN' && editor.dom.getAttrib(selectedNode, 'class').indexOf('fnoteWrap') !== -1;

        var selectIndex = (function(){
            if (selectedNode.className.indexOf('fnoteWrap') !== -1) {
        ...
        if (isFootNotes) {
            name = selectedNode.name || decodeURIComponent(selectedNode.getAttribute('data-nota')) || '';
        }
        ...
        editor.windowManager.open({
            ...
            onSubmit: function(e) {
                var newfootnoteContent = e.data.name,
                    ...
                    htmlTemplate = '<span class="fnoteWrap nota" data-nota="'+newfootnoteContent+'" id="#wk_ft{FOOTNOTE_INDEX}" contenteditable="false"><button class="fnoteBtn">' + editor.selection.getContent({ format: 'text' }) + '</button></span>',

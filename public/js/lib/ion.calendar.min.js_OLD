...
function getDiarios() {
    return $.ajax({
        type: "GET",
        url: $('#form-calendario').attr('action') + 'get_diaries',
        async: false
    }).responseText;
}
                    ...
                    if(settings.sundayFirst) {
                        ...
                        for(i = 1; i <= monthLastDay; i++) {
                            ...
                            if(moment(timeNowLocal).date(i).format("D.M.YYYY") === timeNow.format("D.M.YYYY")) {
                                ...
                                if(mes < 10){
                                    if(i < 10){
                                        data_amiga = "0"+i.toString()+"0"+mes.toString()+ano.toString();
                                    }else{
                                        data_amiga = i.toString()+"0"+mes.toString()+ano.toString();
                                    }
                                }else{
                                    if(i < 10){
                                        data_amiga = "0"+i.toString()+mes.toString()+ano.toString();
                                    }else{
                                        data_amiga = i.toString()+mes.toString()+ano.toString();
                                    }
                                }
                                var has_diary = false;
                                for(item in items){
                                    if (items[item] == data_amiga){
                                        has_diary = true;
                                    }
                                }
                                if(has_diary)
                                    html += '<td class="ic__day has_diary">' + i + '</td>';
                                else

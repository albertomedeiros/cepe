...
abstract class Fishy_Controller
{
	...
	public final function __construct($route = null)
	{
		...
		//load helpers
		$glob = glob(FISHY_HELPERS_PATH . "/*.php");

		if ($glob) {
			foreach ($glob as $helper) {
				require_once $helper;
			}
		}

		$glob = glob(FISHY_SLICES_PATH . "/*/app/helpers/*.php");
	...
	public static function run($route) {
		...
		if (!class_exists($controller_name)) {
		    dispatch_error(new Exception("Controller {$controller_name} ==> {$method} doesn't exists"), 404);
		}
	...
	{	
		...
		if ($object->$field) {
			$format = pathinfo($object->$field, PATHINFO_EXTENSION);
			...
			if ($this->isMobile && !$this->isTablet && $configuration['width'] > 900) {
			...
			$file = FISHY_PUBLIC_PATH . '/' . $object->$field;

			if (file_exists($path) && file_exists($file) && filemtime($file) > filemtime($path)) {
				unlink($path);
			}
		...
		header("Content-Type: " . $mime_types[$configuration['format']]);

...
class ActiveRelationMany extends ActiveRelation
{
		...
		if (isset($this->options['through'])) {
			...
			if (isset($this->options['module'])) {
				...
				$sql = "
					SELECT
						`{$foreign_table}`.id,
						`{$foreign_table}`.gallery_id,
						`{$foreign_table}`.object_type_id,
						`{$foreign_table}`.description
					FROM
						`{$foreign_table}`
						right join `{$rel_table}` on `{$rel_table}`.`{$foreign_field}` = `{$foreign_table}`.`{$foreign_primary_key}`
					WHERE
						`{$rel_table}`.`{$local_field}` = '{$id}' AND `module` = '{$module}'
				";
			} else {
				if (isset($this->options['self-referencing'])) {
					$sql = "
						SELECT
							`{$foreign_table}`.*
						FROM
							`{$foreign_table}`
						WHERE
							`{$foreign_table}`.`{$foreign_field}` = '{$id}'
					";
				} else {
					$sql = "
						SELECT
	...
	protected function add_relation($object)
	{
		if (isset($this->options['through'])) {
			...
			if (isset($this->options['module'])) {
				$module = $this->options['module'];
				DbCommand::execute("INSERT INTO `{$rel_table}` (`{$local_field}`, `{$foreign_field}`, `module`) VALUES ('{$local_id}', '{$foreign_id}', '{$module}')");
			} else {

...
class ActiveRecord_Validators
{
	public static $validations_messages = array(
		...
		'invalid_numberrange' => 'The number in field %s is out of a range',
		...
		'invalid_extension' => 'This file field %s no have the accepted extension'
	);
	...
	/**
	 * Validates if a field contains a number inside a range
	 *
	 * @param ActiveRecord $object Object to be tested
	 * @param string $field Field to test
	 * @return boolean
	 * @author robert
	 */
	public static function validates_numericalityrange_of($object, $field, $min, $max, $err = null)
	{
		if (!is_numeric($object->$field)) {
			$object->add_error($field, $err ? $err : sprintf(self::$validations_messages['invalid_number'], $field));
			return false;
		...
		if ($object->$field < $min) {
			$object->add_error($field, $err ? $err : sprintf(self::$validations_messages['invalid_numberrange'], $field));
			return false;
		}
		if ($object->$field > $max) {
			$object->add_error($field, $err ? $err : sprintf(self::$validations_messages['invalid_numberrange'], $field));
	...
	// public static function validates_extension_of($object, $field, $list, $err = null)
	// {
	// 	if(!in_array($object->{$field}, $object->{'extensions_' . $field}))
	// 	{
	// 		$object->add_error($field, $err ? $err : sprintf(self::$validations_messages['invalid_extension'], $field));
	// 		return false;
	// 	}
	// 	return true;
	// }

...
class FieldAct
{
	...
	/**
	 * Automatic upload zip file for a html5 field
	 *
	 * Example:
	 *   $this->field_as_html5("my_file_field", array("public" => true));
	 *
	 * Properties:
	 * - public: determine if the file will be uploaded into a public location, setting
	 *           this property to true you can easly access the uploaded file by
	 ...
	 *           option when you are not dealing with private files
	 */
	private static function _set_html5($object, $field, $value, $configuration = array())
	{
		$configuration = array_merge(array(
			"public" => false
		), $configuration);
		...
		$new_name = self::normalize_filename($value['name']);
		$new_name = substr($new_name, 0, -4).'/index.html';
		...
		$zip = new ZipArchive;
		$zip->open($value['tmp_name']);
		$zip->extractTo(dirname($new_path));
		$zip->close();

...
class Fishy_StringHelper
{
	...
	{ 
		...
		$slug = strtolower(str_replace($find, $replace, $field)); 
		
		return $slug; 
	}

	/* Editado por 4job - Status das Inscrições */
	public static function status_subscription($id) {
		
		$status = array(0 => 'Inabilitado', 1 => 'Habilitado', 2 => 'Cancelada');

		if ($id == "") { $return = 'Pendente'; } else { $return = $status[$id]; }

		return $return;
	}

	/* Editado por 4job - Status dos Logs das Inscrições */
	public static function status_subscription_log($id) {
		
		$status = array(0 => 'Inabilitado', 1 => 'Inscrito', 2 => 'Cancelada', 3 => 'Habilitado');

		if ($id == "") { $return = ''; } else { $return = $status[$id]; }

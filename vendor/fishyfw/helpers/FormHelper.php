...
class Fishy_FormHelper
{
	...
	public static function text_number_field($field, $options = array(), $html_options = array())
	{
		$object = self::get_object();
		$fieldname = self::get_field($field);
		$fieldid = self::get_field_id($field);
	...
	public static function checkbox_tree($field, $roots, $options = array(), $html_options = array())
	{
		...
		foreach ($roots as $item) {
			...
			$content .= self::build_tag('label', array(), $tag);
		}
		
		return self::build_tag('div', array('class' => 'list-permits'), $content);
	}

...
class Bidding extends ActiveRecord
{
    public function setup()
    {
        ...
        $this->has_many('BiddingFile as files');
        $this->has_many('registers', array('through' => 'bidding_registers', 'local_field' => 'bidding_id'));
    }
    ...
    public function before_save()
    {
        ...
        if ($this->slug != $slug) {
            $count = ActiveRecord::model('Bidding')->count(array('conditions' => 'slug like "' . $slug . '%"'));

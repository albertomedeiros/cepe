...
class News extends ActiveRecord
{
    public static $presets = array(
         'image' => array('default' => '570x380x1', 'gwm' => '114x76x1')
    );
    ...
    public function setup()
    {
        ...
        $this->field_as_file('image', array('public' => true));

        $this->has_many('NewsFile as files');
        $this->has_many('categories', array('through' => 'news_categories', 'local_field' => 'news_id'));
    }

    public function before_destroy()
    {
        @unlink(FISHY_PUBLIC_PATH . '/' . $this->image);
    ...
    public function before_save()
    {
        ...
        if ($this->slug != $slug) {
            $count = ActiveRecord::model('News')->count(array('conditions' => 'slug like "' . $slug . '%"'));

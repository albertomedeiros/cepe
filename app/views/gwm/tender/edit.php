<?php $this->title_bar = '<li><a href="' . $this->site_url('tender') . '">Concursos</a>/</li><li><span>' . (!$this->object->exists() ? 'Adicionar novo' : 'Editar') . '</span></li>' ?>

<h2><?= !$this->object->exists() ? 'Inserir' : 'Editar' ?> concurso</h2>
  ...
  <fieldset>
   ...
   <div class="row">
    <div class="span4" style="margin-bottom:15px;">
     <label for="text_field">Texto <?= Fishy_FormHelper::text_area('text', null, array('class' => 'tinymce')) ?></label>
   </div>
 </div>
...
<a href="<?= $this->site_url('tender') ?>" class="cancel-form">Cancelar</a>
